...
class AbstractGenerator(Sequence, ABC):
    """
...
class MethylationArrayGenerator(AbstractGenerator):
    """
    This class creates a generator for tensorflow starting from a methylation array
    """
    ...
    def __init__(self, df_source, label_column, batch_size=32, shuffle=True):
        ...
        """
        if type(df_source) == str:
            df = pickle.load(open(df_source, "rb"))
        else:
            df = df_source
        df["pheno"] = pd.get_dummies(df["pheno"][label_column])
        barcodes = list(df["pheno"].index.values)
        super().__init__(df, barcodes, batch_size, shuffle)
        self.on_epoch_end()
    ...
    def __data_generation(self, step_barcodes):
        ...
        :return: Two numpy array containing features and labels
        """
        return self.data()["beta"].loc[step_barcodes].to_numpy(), self.data()["pheno"].loc[step_barcodes].to_numpy()
