...
logfile_name = "../data/cnv_embedding.tsv"
...
dataset = pickle.load(open("../data/cnv_exp.pkl", "rb")).rename(columns=lambda g: g.split('.')[0])
...
over_rate_genes = set(over_rate_genes).intersection(set(dataset.columns.values))
...
cnv_to_encode = pickle.load(open("../data/cnv_exp_ma.pkl", "rb"))
cnv_to_encode["beta"] = cnv_to_encode["beta"].rename(columns=lambda g: g.split('.')[0])[over_rate_genes]
cnv_dataset = cnv_encoder.encode_methylation_array(cnv_to_encode)
pickle.dump(cnv_dataset, open("../data/cnv_embedded_pam.pkl", "wb"))
...
to_remove = list()
for index, row in cnv_dataset["pheno"].iterrows():
    if row["subtype"] != "control":
        ...
        if barcode in gt_index and gt_check.loc[barcode]["PAM50 subtype"] != "Normal-like":
            cnv_dataset["pheno"].at[index, "subtype"] = gt_check.loc[barcode]["PAM50 subtype"]
        else:
...
with open(logfile_name, 'a') as logfile:
    base = "{}\t{}\t{}\t{}\t{}\n"
    logfile.write(base.format(str(cnv_encoder), "MLP", ld, val_res, test_res))
    logfile.write(base.format(str(cnv_encoder), "SVM", ld, svm_val, svm_test))
    logfile.write(base.format(str(cnv_encoder), "KNN", ld, knn_val, knn_test))
    logfile.write(base.format(str(cnv_encoder), "RF", ld, rf_val, rf_test))
